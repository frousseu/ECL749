...
Ce premier exercice consiste à importer une base de données en format *.txt*. Cette base de données contient des mesures morphométriques d'une espèce de bruant (un oiseau). Vous pouvez la télécharger à l'adresse suivante: [https://raw.githubusercontent.com/frousseu/ECL749/master/data/sparrows.txt](https://raw.githubusercontent.com/frousseu/ECL749/master/data/sparrows.txt). Vous devez utiliser le bouton droit de la souris pour sauvegarder le contenu de la page dans un fichier *.txt*.
...
2) Explorez et illustrez les liens entre la hauteur et la largeur et les différentes variables explicatives (utilisez `boxplot`ainsi que la fonction `par` et son argument `mfrow` pour fractionner votre fenêtre graphique).

3) Effectuez une ANOVA afin d'étudier l'effet du broutement, de la couverture arbustive, du fertilisant ou de l'espacement sur la hauteur et la largeur des individus. Pour ce faire, vous pouvez utiliser les fonctions `lm` ou `aov` en combinaison avec la fonction `summary`. Interprétez également les résultats.
...
1) Étudiez la distribution et les relations entre vos variables

2) Illustrez spatialement les présences/absences

3) Construisez et interprétez votre un modèle permettant d'expliquer la présence de l'espèce

4) Utilisez la fonction `predict` ou le package **visreg** pour illustrez les prédictions de votre modèle

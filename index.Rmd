...
Pour cet atelier, vous vous pratiquerez à présenter des méthodes et des résultats. Vous devrez effectuer l'analyse de deux bases de données, présenter les méthodes utilisées et présenter les résultats de vos analyses. Puisque vous n'avez pas énormément d'informations sur la façon dont les données ont été récoltés, vous vous concentrerez surtout sur les méthodes statistiques qui ont été utilisées pour effectuer vos analyses. 
...
Dans ce projet, un des objectifs était de déterminer l'efficacité de lumières dissuasives pour réduire la prédation du bétail par les prédateurs (renards, cougars). Pour ce faire, des caméras-trappes ont été installées dans un certain nombre de champs afin d'enregistrer la présence de prédateurs. Le nombre de visites de prédateurs enregistrées en présence ou en absence de lumières dissuasives par chaque caméra a été compilé afin de déterminer si les lumières ont un effet négatif sur le nombre de visites par les prédateurs. Dans la base de données [lumiere_predateur](https://raw.githubusercontent.com/frousseu/ECL749/master/data/lumiere_predateur.csv), vous avez la présence (1) ou l'absence (0) de lumières, l'identifiant du **champ**, l'identifiant de la **camera**, le nombre de visites par des prédateurs (**nbpred**) et le nombre de jours avec ou sans lumière (**nbjour**). Ces données ont été récoltés à l'été 2019 par un étudiant en stage.
...
Voici ce que je présenterais concernant cette analyse dans les sections méthodes et résultats. Encore ici, cet exemple donne une bonne idée de quoi il faudrait présenter dans ces sections, mais ce n'est pas nécessairement un exemple absolu! 
